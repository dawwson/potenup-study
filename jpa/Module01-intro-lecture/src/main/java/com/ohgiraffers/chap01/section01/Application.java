...
public class Application {
     ...
     * [1단계 문제]
     * 문제 상황 :
     * 우리는 데이터베이스에서 ID가 1인 "학생" 역할을 두 번 조회했다.
     * 우리의 상식으로는 "roles"와 "roles2"는 같은 회원이므로, 두 객체는 동일해야 한다.
     * 하지만 결과는 "false"가 나오게 된다.
     *
     * 문제의 원인 :
     * 패러다임 불일치 1: 객체의 동일성 vs 동등성
     * - 객체지향 세상 : == 비굔ㄴ 두 객체가 메모리 상에서 같은 존재인지를 묻는다.
     * - 데이터베이스 세상 : role_id = 1이라는 조건은 "값이 같은 데이터"를 의미한다.
     *
     * JDBC를 사용하면 DB에서 조회할 때마다 새로운 Roles 객체를 메모리에 만들기 때문에
     * 값은 같지만(동등성), 서로 다른 존재(동일성 X)가 되어버린다.
     *
     * 이 문제를 해결하지 않으면 :
     * 만약 roles 객체의 이름을 바꾸고 DB에 저장한 뒤, 아무것도 모르는 roles2 객체를 다시 저장하면
     * "roles"의 변경사항이 덮어씌워지는 끔찍한 데이터 불일치 문제가 발생할 수 있다.
     *
     * JPA는 이것을 어떻게 해결하는지 이해하는 것이 이번 모듈의 핵심이다.
     * @param args
     */
    public static void main(String[] args) {
        ...
        System.out.println("ID가 1인 두 Role 객체는 과연 같은 존재일까요? => " + (roles == roles2)); // false
